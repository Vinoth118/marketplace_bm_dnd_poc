...
import { FeatureWithIconControlledWidgetData } from "../../builder_types";
...
type FeatureWithIconControlledWidgetDataFormProps = {
    initialData: FeatureWithIconControlledWidgetData
}

const FeatureWithIconControlledWidgetDataForm = forwardRef<{ getFormData: () => FeatureWithIconControlledWidgetData }, FeatureWithIconControlledWidgetDataFormProps>(({ initialData }, forwardedRef) => {
    const [widgetName, setWidgetName] = useState(initialData?.customWidgetName ?? '');
    const [data, setData] = useState({ list: covertImageWidgetDataToStateDataType([...(initialData?.data ?? [])], 'FEATURE_WITH_ICON_CONTROLLED').map(e => ({ ...e, id: e.id ?? uuidv4() })) })

    useEffect(() => setData({ list: covertImageWidgetDataToStateDataType([...(initialData?.data ?? [])], 'FEATURE_WITH_ICON_CONTROLLED').map(e => ({ ...e, id: e.id ?? uuidv4() })) }), [initialData])
    ...
    const onClickSubmit = () => {
        const submitData = {
            ...
            data: data.list.filter(e => e.path.trim() != '').map(({ id, path, additional_field_1 }) => ({ id, path, feature: additional_field_1 }))
        }
        return submitData;
    }
...
export default FeatureWithIconControlledWidgetDataForm;